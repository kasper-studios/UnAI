...
3. **Installed Workspace & Data** (Установленный у пользователя экземпляр)  
   - Код: `~/.unai/workspaces/<id>/` (глобально для машины — MCP-сервер один, он не знает «из какого проекта» идёт вызов);
...
4. Воркспейс появляется в `unai workspace ls` и его capability — в MCP-тулсах.

> Замечание: воркспейсы живут глобально (в `~/.unai/`), потому что MCP-сервер — один на машину и не знает «из какого проекта» поступает вызов. Enable/disable управляет только тем, виден ли воркспейс в MCP-тулсах; никаких постоянных сервисов/демонов нет — активация происходит on-demand при MCP-вызове.
